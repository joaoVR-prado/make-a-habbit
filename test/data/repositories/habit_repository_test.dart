...
void main() {
  ...
  group('TESTES BÁSICOS DE ADIÇÃO, REMOÇÃO E EDIÇÃO DE HÁBITOS', () {
    test(
      'Ao adicionarmos um hábito, a lista de hábitos deve ser atualizada',
      () async {
        ...
        when(
          () => mockNotifications.save(newHabit.id, newNotification),
        ).thenAnswer((_) async {});
        ...
        final currentList = providerContainer
            .read(habitControllerProvider)
            .requireValue;
        expect(
          currentList.length,
          1,
          reason: 'A lista de hábitos agora deve conter 1 tem (novo hábito)',
        );
        expect(
          currentList.first.name,
          newHabit.name,
          reason: 'O nome do hábito que acabou de ser salvo deve estar correto',
        );
        expect(
          currentList.first.conclusionType,
          newHabit.conclusionType,
          reason:
              'O tipo de conclusao do hábito que acabou de ser salvo deve estar correto',
        );
        expect(
          currentList.first.goalQuantity,
          newHabit.goalQuantity,
          reason:
              'A meta de conclusão do hábito que acabou de ser salvo deve estar correto',
        );
        expect(
          currentList.first.frequency.type,
          newHabit.frequency.type,
          reason: 'O nome do hábito que acabou de ser salvo deve estar correto',
        );
    ...
    test(
      'Ao removermos um hábito, a lista de hábitos deve ser atualizada',
      () async {
        final newHabit = HabitModel(
          id: '1',
          iconCode: 0,
        ...
        when(
          () => mockConclusions.deleteByHabit(newHabit.id),
        ).thenAnswer((_) async {});
        when(
          () => mockNotifications.delete(newHabit.id),
        ).thenAnswer((_) async {});
        ...
        expect(
          providerContainer.read(habitControllerProvider).requireValue.length,
          1,
        );
        ...
        expect(
          currentList.length,
          0,
          reason: "A lista deve estar vazia após deletar o hábito",
        );
    ...
    test('Ao editarmos um hábito, os dados do hábito atual devem ser '
      ...
      final editedNewHabit = HabitModel(
        ...
        iconCode: 1,
        name:
            'Diminuir o café para 4 xícaras ao dia, todos os dias exceto final de semana',
        conclusionType: HabitConclusionType.goalQuantity,
        goalQuantity: 4,
        frequency: HabitFrequency.fromType(
      ...
      when(
        () => mockRepository.update(editedNewHabit),
      ).thenAnswer((_) async {});
      when(
        () => mockRepository.getById(editedNewHabit.id),
      ).thenReturn(newHabit);
      when(
        () => mockNotifications.save(newHabit.id, newNotification),
      ).thenAnswer((_) async {});
      ...
      expect(
        providerContainer.read(habitControllerProvider).requireValue.first.name,
        'Diminuir o café para 2 xícaras ao dia, todos os dias',
      );
      ...
      expect(
        habitInCurrentList.id,
        equals('1'),
        reason: 'O id não deve ser alterado',
      );
      expect(
        habitInCurrentList.iconCode,
        1,
        reason:
            'O icone deve ser alterado, de acordo com o que o usuário inseriu',
      );
      expect(
        habitInCurrentList.name,
        equals(
          'Diminuir o café para 4 xícaras ao dia, todos os dias exceto final de semana',
        ),
      );
      expect(
        habitInCurrentList.conclusionType,
        HabitConclusionType.goalQuantity,
        reason:
            'O método de conclusão do hábito não foi alterado, então deve ser o mesmo',
      );
      expect(
        habitInCurrentList.goalQuantity,
        4,
        reason: 'Quantidade para concluir o hábito foi alterado',
      );
      expect(
        habitInCurrentList.frequency.type,
        HabitFrequencyType.weekly,
        reason: 'O tipo de frequência deve ser alterado para semanal',
      );
      expect(
        habitInCurrentList.frequency.selectedDays,
        equals([1, 2, 3, 4, 5]),
        reason: 'Os dias do hábito foram alterados',
      );
    ...
    test(
      'Ao deletarmos todos os hábitos, a lista de hábitos deve estar vazia',
      () async {
        ...
        expect(
          currentList.length,
          0,
          reason: "A lista deve estar vazia após deletar todos os hábitos",
        );
  ...
  group('TESTES COM BASE NAS DATAS ', () {
    test(
      'Hábitos não devem aparecer no dia atual se a data de inicio ainda não chegou',
      () async {
        ...
        expect(
          isActive,
          isFalse,
          reason: 'Hábitos que ainda não começaram não devem aparecer na lista',
        );
      },
    );

    test(
      'Hábitos não devem aparecer no dia atual se a data de fim já passou',
      () async {
        final today = DateTime(2026, 01, 25);
        ...
        expect(
          isActive,
          isFalse,
          reason: 'Hábitos que já acabaram não devem aparecer na lista',
        );
      },
    );

    test(
      'Hábitos criados com a data de hoje já devem aparecer na lista ',
      () async {
        final today = DateTime(2026, 01, 25);
        ...
        expect(
          ...
          isTrue,
          reason:
              'Hábitos criados hoje com a data de inicio para hoje devem aparecer na lista',
        );
      },
    );

    test(
      'Verificamos quais dias o hábito semanal deve aparecer nas datas fornecidas',
      () async {
        ...
        expect(
          ...
          isTrue,
          reason:
              'O usuário criou um hábito que deve aparecer na segunda, quinta, sabado e domingo. Como a data fornecida é domingo, o hábito deve aparecer hoje',
        );
        expect(
          isActiveTuesday,
          isFalse,
          reason:
              'O usuário criou um hábito que deve aparecer na segunda, quinta, sabado e domingo. Como a data fornecida é terça, o hábito não deve aparecer',
        );
      },
    );

    test(
      'Verificamos quais dias o hábito mensal deve aparecer, com base nas dastas fornecidas',
      () async {
        final startDate = DateTime(2026, 01, 25);
        final today = DateTime(2026, 02, 15);
        ...
        expect(
          ...
          isTrue,
          reason:
              'O usuário criou um hábito que deve aparecer no dia 1, 15 e 30 de cada mês. Como a data fornecida é dia 15, o hábito deve aparecer hoje',
        );
        expect(
          isActiveFebruary,
          isFalse,
          reason:
              'O usuário criou um hábito que deve aparecer no dia 1, 15 e 30 de cada mês. Como fevereiro não tem dia 30, o hábito não deve aparecer nessa data',
        );
      },
    );
  });
  ...
  group('TESTES DE ESTADO DO RASCUNHO', () {
    ...
    test('O estado inicial do rascunho desve ser vazio', () {
      ...
      expect(
        draftState.existingId,
        isNull,
        reason: 'O ID ainda não foi criado, então deve estar vazio',
      );
      expect(
        draftState.name,
        isEmpty,
        reason: 'O Nome ainda não foi criado, então deve estar vazio',
      );
      expect(
        draftState.category,
        isNull,
        reason: 'A Categoria ainda não foi criada, então deve estar vazia',
      );
      expect(
        draftState.conclusionType,
        isNull,
        reason:
            'O Método de Conclusão ainda não foi criado, então deve estar vazio',
      );
      expect(
        draftState.isStreakEnabled,
        isFalse,
        reason: 'A ofensiva por padrão vem desativada',
      );
      expect(
        draftState.weeklyDays,
        isEmpty,
        reason:
            'Não temos um hábito criado, então lógicamente não temos dias selecionados',
      );
      expect(
        draftState.monthlyDays,
        isEmpty,
        reason:
            'Não temos um hábito criado, então lógicamente não temos dias selecionados',
      );
    ...
    test(
      'O Notifier deve atualizar as propriedades individuais corretamente',
      () {
    ...
    test(
      'O método clear() deve limpar todos os valores para o valor inicial',
      () {
        final notifier = container.read(draftHabitProvider.notifier);
        ...
        expect(
          clearedState.startDate?.day,
          today.day,
          reason: 'A data de inicio deve voltar para hoje',
        );
        expect(clearedState.startDate?.month, today.month);
        expect(clearedState.startDate?.year, today.year);

        expect(
          clearedState.goalQuantity,
          isEmpty,
          reason: 'A meta deve estar vazia',
        );
      },
    );

    test(
      'O estado inicial de edição deve preencher o rascunho com os dados do hábito',
      () {
        // Criamos um Hábito
